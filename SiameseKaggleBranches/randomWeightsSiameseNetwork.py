...
def gen_model(params):
    ...
    def base_network():
        ...
        for i in range(int(params['n_conv_layers'])):
            if i != 0:
                print(i)
                submodel.add(Conv2D(int(params['no_filters_conv']), (int(params['dim_conv_kernel']),
                ...
                submodel.add(MaxPooling2D(pool_size=(2, 2), strides=None, padding='valid', data_format=None))
            # model.add(Dense(params['outputDense'])) #, kernel_initializer = 'random_uniform', bias_initializer='zeros', activation = 'relu'
        submodel.add(Dense(int(params['outputDense']), activation='relu'))
